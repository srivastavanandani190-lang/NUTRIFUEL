...
interface AuthContextType {
  ...
  loading: boolean;
  signInWithEmail: (emailOrUsername: string, password: string) => Promise<{ error: Error | null }>;
  signUpWithEmail: (email: string, username: string, password: string) => Promise<{ error: Error | null }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInWithEmail = async (emailOrUsername: string, password: string) => {
    try {
      ...
      if (!isEmail) {
        // If username, convert to email format
        email = `${emailOrUsername}@miaoda.com`;
      }
